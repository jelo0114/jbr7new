-- fix_images_complete.sql
-- Complete SQL to fix images in items table
-- Run this after add_images_column.sql

USE `jbr7_db`;

-- Step 1: Add images column (run this first if column doesn't exist)
-- ALTER TABLE `items` ADD COLUMN `images` JSON NULL AFTER `image`;

-- Step 2: Update all items with complete image arrays from image-manifest.json
