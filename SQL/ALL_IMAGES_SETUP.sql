-- ALL_IMAGES_SETUP.sql
-- Complete setup for images - Run this file to fix all image display issues
-- This file includes everything needed to set up images properly

USE `jbr7_db`;

-- ============================================
-- STEP 1: Add images column to items table
-- ============================================
-- Note: If column already exists, skip this step or comment it out
ALTER TABLE `items` ADD COLUMN `images` JSON NULL AFTER `image`;

-- ============================================
-- STEP 2: Update all items with complete image arrays
-- ============================================
...
-- Two Colored Brass Cotton Back Pack
UPDATE `items` 
SET `images` = JSON_ARRAY(
...
-- For any items without images array, use the main image
UPDATE `items` 
SET `images` = JSON_ARRAY(`image`)
WHERE (`images` IS NULL OR JSON_LENGTH(`images`) = 0) AND `image` IS NOT NULL;

-- ============================================
-- Done! All items now have their complete image sets
-- ============================================
